...
export async function listEnfermedadLaboral(params = {}) {
  const { data } = await api.get("/api/v1/talento/enfermedad-laboral/", { params });
  return data?.items ?? data;
}
...
export async function saveEnfermedadLaboral(payload) {
  const { data } = await api.post("/api/v1/talento/enfermedad-laboral/", payload);
  return data;
}
...
export async function listReporteMinisterio(params = {}) {
  const { data } = await api.get("/api/v1/talento/reporte-ministerio/", { params });
  return data?.items ?? data;
}
...
export async function saveReporteMinisterio(payload) {
  const { data } = await api.post("/api/v1/talento/reporte-ministerio/", payload);
  return data;
}

/* ===============RESTRICCIONES LABORALES=============== */
export async function listRestriccionesLaborales(params = {}) {
  const { data } = await api.get("/api/v1/talento/restricciones-laborales/", { params });
  return data?.items ?? data;
}
...
export async function saveRestriccionLaboral(payload) {
  const { data } = await api.post("/api/v1/talento/restricciones-laborales/", payload);
  return data;
}
