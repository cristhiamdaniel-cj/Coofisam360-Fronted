...
export default function GestionesTable() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await listPlanTrabajoRows({ anio: selectedYear, limit: 1000 });
    ...
    try {
      ...
      for (const i of idxs) {
        ...
        for (let m = 1; m <= 12; m++) {
          ...
          const changed = !r._orig || r._orig[2 * (m - 1)] !== pBool || r._orig[2 * (m - 1) + 1] !== eBool;
      ...
      setEditedRows({});
      const fresh = await listPlanTrabajoRows({ anio: selectedYear, limit: 1000 });
  ...
  return (
    <main className="pt-4 pb-0 px-12 overflow-auto">
      <h1 className="titulo-tabla-cupos text-3xl font-semibold pb-4">Plan de Trabajo Anual</h1>
      {statusMsg && (
      ...
      <div className="actions-container flex justify-end mb-4">
        <div className="flex gap-4 items-center">
          ...
          {Object.keys(editedRows).length > 0 && (
            <button onClick={handleSave} disabled={saving} className="action-button flex gap-2 items-center justify-center cursor-pointer">
              {saving ? 'Guardando...' : 'Guardar cambios'}
              <FaRegSave />
            </button>
          )}
          <button className="action-button flex gap-2 items-center justify-center cursor-pointer" onClick={handleDownload}>
            Descargar <FiDownload />
          </button>
          <button className="action-button flex gap-2 items-center justify-center cursor-pointer" onClick={handleAddRow}>
        ...
        <table className="table-auto border-collapse w-full">
          <thead>
            <tr className="tabla-header">
              <th rowSpan="3" className="p-4 border text-center bg-white z-20 min-w-[150px]">Ciclo</th>
              <th rowSpan="3" className="p-4 border text-center bg-white z-20 min-w-[300px]">Actividad</th>
              <th colSpan="24" className="border text-center p-2 bg-white z-20">CRONOGRAMA VIGENCIA</th>
            ...
            <tr>
              {["ENERO","FEBRERO","MARZO","ABRIL","MAYO","JUNIO","JULIO","AGOSTO","SEPTIEMBRE","OCTUBRE","NOVIEMBRE","DICIEMBRE"].map(m => (
                <th colSpan="2" className="border text-center p-2 text-[10px] min-w-[80px] text-white  bg-white z-20" key={m}>{m}</th>
              ))}
              <th rowSpan="2" colSpan="1" className="border text-center p-2 min-w-[150px] bg-white text-white z-20">admin</th>
              <th rowSpan="2" colSpan="1" className="border text-center p-2 min-w-[150px] bg-white text-white z-20">finan</th>
            </tr>
            <tr>
              {Array.from({ length: 12 }).map((_, i) => (
                <>
                  <th key={`p-${i}`} className="border text-center p-2 bg-white z-20 text-white">P</th>
                  <th key={`e-${i}`} className="border text-center p-2 bg-white z-20 text-white">E</th>
                </>
              ))}
            </tr>
          </thead>
            ...
            {actividades.map((actividad, idx) => (
              <tr key={idx}>
                <td className="p-2 border text-center min-w-[150px]">
                  <select value={actividad.ciclo || ''} onChange={e => handleChange(idx, 'ciclo', e.target.value)} className="px-2 py-1 w-full border">
                ...
                <td className="p-2 border text-left min-w-[300px]">
                  <input type="text" value={actividad.actividad || actividad.nombre || ''} onChange={e => handleChange(idx, 'actividad', e.target.value)} placeholder="Nueva actividad" className="px-2 py-1 w-full border" />
                </td>
                {Array.from({ length: 24 }).map((_, i) => (
                  <td key={i} className="p-2 border text-center max-w-[40px]">
                    <input type="checkbox" checked={!!actividad[`col${i}`]} onChange={e => handleChange(idx, `col${i}`, e.target.checked)} className="mx-auto" />
                  </td>
                ))}
                <td className="p-2 border text-center">
                  <select value={actividad.responsable || ""} onChange={e => handleChange(idx, "responsable", e.target.value)} className="w-full border px-1 py-1">
                    {["Comité de Convivencia Laboral","Comité de COPASST","Auxiliar SST","ARL","Coordinador Talento y Cultura","Subgerencia de Innovación Empresarial"].map(opt => (
                      <option key={opt} value={opt}>{opt}</option>
                    ))}
                  </select>
                </td>
                <td className="p-2 border text-center">
                  <select value={actividad.recurso1 || "SI"} onChange={e => handleChange(idx, "recurso1", e.target.value)} className="border rounded text-center p-1 w-full">
                    {["SI", "NO"].map(opt => (<option key={opt} value={opt}>{opt}</option>))}
                  </select>
                </td>
                <td className="p-2 border text-center">
                  <select value={actividad.recurso2 || "NO"} onChange={e => handleChange(idx, "recurso2", e.target.value)} className="border rounded text-center p-1 w-full">
                    {["SI", "NO"].map(opt => (<option key={opt} value={opt}>{opt}</option>))}
                  </select>
                </td>
              </tr>
    ...
    <div className="mt-8">
        ...
        <table className="table-auto border-collapse w-full">
          <thead>
            <tr className="tabla-header">
              ...
              {months.map(m => (
                <th key={m} className="p-3 border text-center whitespace-nowrap text-xs">{m.substring(0,3)}</th>
              ))}
          ...
          <tbody>
            <tr>
              ...
              {resumen.planeadoMes.map((v, i) => (
                <td key={i} className="p-2 border text-center">{v}</td>
              ))}
            ...
            <tr>
              ...
              {months.map((_, i) => (
                <td key={i} className="p-2 border text-center">100%</td>
              ))}
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="border p-3">
          <div className="font-semibold mb-2 text-sm">Seguimiento al Cumplimiento del plan de Trabajo del SGSST Vigencia</div>
          <LineChart labels={months} values={resumen.pctMes} />
        </div>
        <div className="border p-3">
          <div className="font-semibold mb-2 text-sm">Programado vs Ejecutado (anual)</div>
          <BarChartTwo a={resumen.totalEjecutado} b={resumen.totalPlaneado} labelA="Ejecutado" labelB="Programado" />
        </div>
      </div>
    </div>
  );
...
function LineChart({ labels = [], values = [], height = 220 }) {
  ...
  const pathD = values.map((v, i) => `${i === 0 ? 'M' : 'L'} ${xs(i)} ${ys(v || 0)}`).join(' ');
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H}>
      ...
      {/* Línea 100% */}
      <line x1={padL} y1={ys(100)} x2={W - padR} y2={ys(100)} stroke="#ddd" strokeDasharray="4 4" />
      {/* Grid ligera */}
      {[25,50,75].map(g => (
      ...
      {values.map((v, i) => (
        <g key={i}>
          <rect x={xs(i) - 3} y={ys(v) - 3} width="6" height="6" fill="#d62728" />
...
function BarChartTwo({ a = 0, b = 0, labelA = 'A', labelB = 'B', height = 220 }) {
  const width = 560;
  const padL = 40, padR = 16, padT = 14, padB = 26;
  const W = width, H = height;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H}>
      ...
      {/* Barras */}
      <rect x={xA} y={y(a)} width={barW} height={H - padB - y(a)} fill="#1f77b4" opacity="0.8" />
      <rect x={xB} y={y(b)} width={barW} height={H - padB - y(b)} fill="#ff9896" stroke="#d62728" strokeDasharray="4 2" opacity="0.7" />
      {/* Etiquetas */}
